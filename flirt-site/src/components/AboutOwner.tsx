    ...
    <section className="section bg-sand overflow-hidden">
      <div className="container-wide grid lg:grid-cols-[1.05fr_1fr] gap-14 lg:gap-20 items-center">
        <div className="order-2 lg:order-1">
          ...
          <Reveal delay={80}>
            <h2 className="display-lg mt-5">
              {about.headlineLine1}{" "}
              <span className="script-accent">{about.headlineScript}</span>
            </h2>
          </Reveal>

          <Reveal delay={140}>
            <p className="mt-4 font-sans text-[0.65rem] uppercase tracking-wide2 text-clay">
              {about.role}
            </p>
          </Reveal>
          ...
          {about.body.map((p, i) => (
            <Reveal key={i} delay={220 + i * 80}>
              <p className="mt-6 text-[1.02rem] leading-[1.85] text-mute">{p}</p>
            </Reveal>
          ))}

          <Reveal delay={480}>
            <p className="script-accent mt-9 !text-[3rem] !text-clay">
              {about.signature}
            </p>
          </Reveal>
        ...
        <Reveal className="relative order-1 lg:order-2" delay={120}>
          <div
            className="absolute -right-4 -top-4 w-full h-full arch border border-shell pointer-events-none hidden sm:block"
            aria-hidden="true"
          />
